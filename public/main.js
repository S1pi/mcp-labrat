...
const layoutButtons = document.querySelectorAll(".preview-btn");
const layoutClasses = [
  "layout-neon-noir",
  "layout-zen-garden",
...
const setLayout = (layout) => {
  if (!layoutClasses.includes(layout)) return;
  ...
  try {
    window.localStorage.setItem("preferredLayout", layout);
  } catch (err) {
    console.warn("Unable to store preferred layout.", err);
  }
};
...
if (layoutButtons.length > 0) {
  ...
  try {
    const savedLayout = window.localStorage.getItem("preferredLayout");
    if (savedLayout) {
      setLayout(savedLayout);
    }
  } catch (err) {
    console.warn("Unable to read preferred layout.", err);
  }
}
